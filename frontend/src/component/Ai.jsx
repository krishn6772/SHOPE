...
function Ai() {
    ...
    const rocognition = new speechRecognition()
    if(!rocognition){
        console.log("not supported")
    }

    rocognition.onresult = (e) => {
        ...
        else if(transcript.toLowerCase().includes("search") || transcript.toLowerCase().includes("collections") || transcript.toLowerCase().includes("product") || transcript.toLowerCase().includes("products")){
  ...
  return (
    <div className='fixed lg:bottom-[20px] md:bottom-[40px] bottom-[80px] left-[2%]' onClick={()=>{rocognition.start();
    openingSound.play();
    setActiveAi(true)
    }}>
      <img src={ai} alt='' className={`w-[100px] cursor-pointer ${activeAi ? 'translate-x-[10%] translate-y-[10%] scale-125' : 'tanslate-x-[0] translate-y-[0] scale-100'} transition-transform`} style={{filter: `${activeAi?"drop-shadow(0px 0px 30px #00d2fc)":"drop-shadow(0px 0px 20px black)"}`}}/>
    </div>
  )
}
